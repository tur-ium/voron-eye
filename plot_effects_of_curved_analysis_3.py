...
if plot_gamma:
    ...
    ax5.plot(np.linspace(xmin,xmax,100),fit_h,"g--",label="{} fit".format(condition))
if plot_lognorm:
    #LOGNORM
    if floc: 
...
if plot_gamma:
    ...
    generalized_ks_test(data,gamma,label="Gamma",floc=floc)
    fit_h = gamma.pdf(np.linspace(xmin,xmax,100),a,loc,scale)
    ax5.plot(np.linspace(xmin,xmax,100),fit_h,"r--",label="{} fit".format(condition))
if plot_lognorm:
    #LOGNORM
    if floc: 
        a,loc,scale = lognorm.fit(data,floc=0) 
...
if plot_gamma:
    ...
    generalized_ks_test(data,gamma,label="Gamma",floc=floc)
    fit_h = gamma.pdf(np.linspace(xmin,xmax,100),a,loc,scale)
    ax5.plot(np.linspace(xmin,xmax,100),fit_h,fmt["color"]+"-",label="{} fit".format(condition))
if plot_lognorm:
    #LOGNORM
    if floc: 
        a,loc,scale = lognorm.fit(data,floc=0) 
...
#    print("---\nGLAUCOMATOUS\n---")
    ...
    if plot_gamma:
        ...
        generalized_ks_test(data,gamma,label="Gamma",floc=floc)
        fit_h = gamma.pdf(np.linspace(xmin,xmax,100),a,loc,scale)
        ax5.plot(np.linspace(xmin,xmax,100),fit_h,"b--",label="{} fit".format(condition))
    if plot_lognorm:
        #LOGNORM
        if floc: 
            a,loc,scale = lognorm.fit(data,floc=0) 
